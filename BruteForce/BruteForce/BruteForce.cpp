...
void BruteForce(char startJump, char letCount, int *x)
{
	char i, j;
	...
	for (i = 0; i <= n && !gasit; i++)
	{
		...
		if (startJump == letCount)	// we got a permutation
		{
			...
			{									// it outputs current stats). x[0] = last 5th letter
				x[0] = x[letCount - 4];
				mu.lock();
				cout << model << " TIME:" << (unsigned int)duration_cast<seconds>(system_clock::now() - refz).count()
					 << "s " << " THR: " << this_thread::get_id() << endl;
				mu.unlock();
			}
			...
			if (model == cautat)	// if starting string matches permutation
			{
				...
				cout << "--------------- A FOST GASIT SIRUL " << model << " in "
					 << duration_cast<seconds>(system_clock::now() - refz).count() 
					 << "s ---------------------" << endl;
				gasit = true;
				mu.unlock();
			}
